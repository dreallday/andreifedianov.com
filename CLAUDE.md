...
Personal **resume site** for Andrei Fedianov — a single-page, statically-generated site built with **Astro 5**. Minimalist typographic design, light/dark theme toggle, amber/gold accent. Zero client JS except a tiny inline theme toggle. Served as static files from a self-hosted **Unraid + nginx** box, fronted by **Cloudflare** at **andreifedianov.com** (the box itself is reached over Tailscale/LAN for deploys). The site was previously hosted on AWS S3 — that Terraform is now parked under `legacy/` (see Deployment).
...
- `src/layouts/Base.astro` — HTML shell, SEO/OpenGraph meta, inline favicon, schema.org **`Person` JSON-LD** (built from `profile`/`contact`), and the **pre-paint theme script** (reads `localStorage.theme` or `prefers-color-scheme` and sets `data-theme` on `<html>` before first paint to avoid a flash).
...
`astro.config.mjs` sets `site` and enables `@astrojs/sitemap` (emits `sitemap-index.xml` at build). `public/robots.txt` is served at the root and points at the sitemap. Combined with the `Person` JSON-LD in `Base.astro`, this covers the SEO surface for a single-page résumé.
...
- Shared classes live in `global.css`: **`.wrap-inner`** (the centered `max-width: var(--maxw)` content container, used by every section) and **`.tag`** (the monospace chip for the skills and per-role tech lists). Reach for these instead of re-defining container/chip styles per component.
- Component-specific styles use Astro's scoped `<style>` blocks — they layer only their extras on top of the shared classes (e.g. `Header` adds `position: relative` to `.wrap-inner`, `Footer` adds flex). Cross-theme rules inside a component reach the root via `:global(:root[data-theme="dark"])`.
- Layout pattern: sections are full-bleed (top border spans the viewport) with an inner `.wrap-inner` holding the content. There's a `@media print` block for clean printing.
...
The public domain **andreifedianov.com** is fronted by **Cloudflare** → the Unraid nginx origin. After a deploy the origin is updated immediately, but the Cloudflare edge cache may still serve the old page; purge the Cloudflare cache if you need the change to appear instantly. Note port 80 on the box is the Unraid webGUI (redirects to `/Main`) — the site is not served there, only via the domain.
